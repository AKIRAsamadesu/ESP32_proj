...
{
	ESP_LOGI(__FUNCTION__, "png_init w=%"PRIu32" h=%"PRIu32, w, h);
	ESP_LOGI(__FUNCTION__, "screenWidth=%d screenHeight=%d", pngle->screenWidth, pngle->screenHeight);
	pngle->imageWidth = w;
	pngle->imageHeight = h;
	pngle->reduction = false;
	pngle->scale_factor = 2.0;
...
// #endif
	...
	uint32_t _x = x;
	uint32_t _y = y;
	// ESP_LOGI(__FUNCTION__, "scale_factor is: %s", pngle->scale_factor);
	if (pngle->reduction) {
